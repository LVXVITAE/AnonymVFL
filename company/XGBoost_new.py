...
class SSXGBoost:
    ...
    def save(self, paths: dict[str, str], ext='npy'):
        ...
        self.company(save_model)(
            weights1, self.FedQuantiles.partitions[self.company], paths['company'])
        self.partner(save_model)(
            weights2, self.FedQuantiles.partitions[self.partner], paths['partner'])
    ...
    def load(self, paths: dict[str, str]):
        ...
        w1, trees1, quantiles1, info1 = self.company(
            load_model, num_returns=4)(paths['company'])
        w2, trees2, quantiles2, info2 = self.partner(
            load_model, num_returns=4)(paths['partner'])
        ...
        self.FedQuantiles = load(
            {self.company: quantiles1, self.partner: quantiles2}, partition_way=PartitionWay.HORIZONTAL)
        self.trees = []
        ...
        for i in range(self.n_estimators):
            t = Tree(self.devices, self.lambda_,
                     self.max_depth, self.div, self.mission)
            t.root = trees1[i]
...
def SSXGBoost_test(dataset):
    ...
    model = SSXGBoost(
        devices={'spu': spu, 'company': company, 'partner': partner})
    model.load({'company': f'SSXGBoost_{dataset}_company',
    ...
    print(
        f"Accuracy of SSXGBoost on {dataset} dataset after loading: {Accuracy:.4f}")
