...
class SSLR(SSML):
    ...
    def dispatch_weight(self):
        ...
        def get_item(arr: jnp.ndarray, keys):
            return arr[keys]
        w1, w2 = self.spu(get_item, static_argnames=['keys'])(self.w, np.arange(self.split_col)), self.spu(
            get_item, static_argnames=['keys'])(self.w, np.arange(self.split_col, self.in_features))
        w1 = w1.to(self.company)
        w2 = w2.to(self.partner)
        w = load({self.company: w1, self.partner: w2},
    ...
    def predict(self, X: FedNdarray, device: PYU) -> PYUObject:
        ...
        assert isinstance(
            device, PYU), 'predictions must be moved to a PYU device'
        ...
        if isinstance(self.w, SPUObject):
            ...
            w = self.w
        z1 = self.company(
            lambda X, w: X @ w)(X.partitions[self.company], w.partitions[self.company]).to(device)
        z2 = self.partner(
            lambda X, w: X @ w)(X.partitions[self.partner], w.partitions[self.partner]).to(device)
        ...
        y = device(to_int_labels, static_argnames=[
                   'threshold'])(y, self.pred_threshold)
    ...
    def fit(self, X: SPUObject, y: SPUObject | PYUObject, X_test: FedNdarray | None = None, y_test: PYUObject | None = None, batch_size=64, val_steps=1, n_epochs=10, lr=0.1, split_col: int = None):
        ...
        """
        assert isinstance(
            X, SPUObject) and X.device == self.spu, "X must be on SPU"
        assert (isinstance(y, SPUObject) and y.device == self.spu) or (
            isinstance(y, PYUObject)), "y must be on active_party PYU or SPU"
        ...
        self.split_col = split_col if split_col is not None else sf.reveal(
            X_test.partition_shape()[self.company])[1]
        ...
        if validate:
            assert isinstance(
                X_test, FedNdarray), "X_test must be a FedNdarray"
        if not self.approx:
        ...
        for j in trange(0, num_samples, batch_size):
            ...
            y_batch = self.train_label_keeper(
                get_item, static_argnames=['keys'])(y, keys)
            Xs.append(X_batch)
            ys.append(y_batch)
        steps = 1
        accs = []
            ...
            for X, y in tqdm((zip(Xs, ys))):
                ...
                if validate and steps % val_steps == 0:
                    ...
                    def compute_f1_metric(y_true: np.ndarray, y_pred: np.ndarray):
                        ...
                        # 使用更安全的比较方式，处理浮点数
                        tp = np.sum((np.abs(y_true - 0.0) < 1e-6) &
                                    (np.abs(y_pred - 0.0) < 1e-6))
                        fp = np.sum((np.abs(y_true - 1.0) < 1e-6) &
                                    (np.abs(y_pred - 0.0) < 1e-6))
                        fn = np.sum((np.abs(y_true - 0.0) < 1e-6) &
                                    (np.abs(y_pred - 1.0) < 1e-6))
                    ...
                    if t == n_epochs:
                        ...
                        if fOr < finalfOr:
                            finalfOr = fOr
                    print(
                        f"Step {steps}, Accuracy: {acc:.4f}, F1: {f1:.4f}, FOR: {fOr:.4f}")
                steps += 1
    ...
    def load(cls, devices, paths):
        ...
        def load_model(path: str):
            ...
            ext = info['save_as']
            if ext == 'csv':
                w = np.loadtxt(os.path.join(path, 'weight.csv'),
                               delimiter=',', ndmin=2)
            else:
                w = np.load(os.path.join(path, 'weight.npy'))
            return w, info
        w1, info1 = devices['company'](
            load_model, num_returns=2)(paths['company'])
        w2, info2 = devices['partner'](
            load_model, num_returns=2)(paths['partner'])
        ...
        model.w = load({model.company: w1, model.partner: w2},
                       partition_way=PartitionWay.HORIZONTAL)
...
def SSLR_test(dataset):
    ...
    test_X = load({company: sf.to(company, test_X[:, :split_col]), partner: sf.to(
        partner, test_X[:, split_col:])})
    ...
    accs = model.fit(train_X, train_y, X_test=test_X, y_test=test_y,
                     n_epochs=10, batch_size=1024, val_steps=10, lr=0.1)
    paths = {
    ...
    test_X = np.hstack([sf.reveal(test_X.partitions[company]),
                       sf.reveal(test_X.partitions[partner])])
