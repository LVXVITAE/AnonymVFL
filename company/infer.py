...
class InferEngine:
    ...
    def compute_intersection(self, company_keys, company_data, partner_keys, partner_data):
        ...
        def filter_data(data: pd.DataFrame, keys: list):
            ...
            # 推理时应与训练特征空间一致：去掉 id，并在存在标签列时去掉 Revenue。
...
def example_run():
    ...
    partner_data, y = partner(lambda data: (
        data.iloc[:, :-1], data.iloc[:, -1].to_numpy()), num_returns=2)(partner_data)

    infer_engine.compute_intersection(
        company_keys, company_data, partner_keys, partner_data)
